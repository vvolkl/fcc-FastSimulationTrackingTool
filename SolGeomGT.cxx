...
void SolGeom::SolGeoFill()
{
	...
	if (fEnable[0])
	{
		...
		fthLay[fNlay] = 0.0005;		// Thickness (meters)
	...
	if (fEnable[1])
	{
		...
		for (Int_t i = 0; i < NlVtx; i++)
		{
			...
			fsgLayU[fNlay] = 20.E-6/R12;				// Resolution Upper side (meters) - 0 = no measurement
			fsgLayL[fNlay] = 20.E-6/R12;				// Resolution Lower side (meters) - 0 = no measurement
	...
	if (fEnable[2])
	{
		...
		for (Int_t i = 0; i < NlVtxo; i++)
		{
			...
			fsgLayU[fNlay] = 50.E-6/R12;				// Resolution Upper side (meters) - 0 = no measurement
			fsgLayL[fNlay] = 50.E-6/R12;				// Resolution Lower side (meters) - 0 = no measurement
			fflLay[fNlay] = kTRUE;				// measurement flag = T, scattering only = F
