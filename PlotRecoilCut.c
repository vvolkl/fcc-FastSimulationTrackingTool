...
void PlotRecoilCut()
{
	TFile *HstF_HZ = new TFile("Hfcut_ZHmm.root", "READ");
	Double_t scale_HZ = 3.28;
	TFile *HstF_ZZ = new TFile("Hfcut_ZZ.root", "READ");
	Double_t scale_ZZ = 68.1;
	TFile *HstF_WW = new TFile("Hfcut_WW.root", "READ");
	...
	// Compare cuts
	//
	TCanvas *Ccos = new TCanvas("Ccos", "angle cuts", 50, 50, 500, 500);
	Ccos->Divide(2, 2);
	TH1D *gHcos = (TH1D *)HstF_HZ->Get("gCos");
	gHcos->Scale(scale_HZ);
	gHcos->SetLineColor(kBlack);
	TH1D *gZcos = (TH1D *)HstF_ZZ->Get("gCos");
	gZcos->Scale(scale_ZZ);
	gZcos->SetLineColor(kBlue);
	TH1D *gWcos = (TH1D *)HstF_WW->Get("gCos");
	gWcos->Scale(scale_WW);
	gWcos->SetLineColor(kGreen);
	Ccos->cd(1);	// Generation level cuts
	gZcos->Draw();
	gHcos->Draw("SAME");
	gWcos->Draw("SAME");
	TLegend *lgCos = new TLegend(0.6, 0.9, 0.9, 0.7);
	lgCos->AddEntry(gHcos, "HZ", "l");
	lgCos->AddEntry(gZcos, "ZZ", "l");
	lgCos->AddEntry(gWcos, "WW", "l");
	lgCos->Draw();
	//
	Ccos->cd(2);
	TH2D *gHpcos = (TH2D *)HstF_HZ->Get("gpCos");
	gHpcos->SetTitle("HZ p-cos correlation");
	gHpcos->Draw("CONT");
	Ccos->cd(3);
	TH2D *gZpcos = (TH2D *)HstF_ZZ->Get("gpCos");
	gZpcos->SetTitle("ZZ p-cos correlation");
	gZpcos->Draw("CONT");
	Ccos->cd(4);
	TH2D *gWpcos = (TH2D *)HstF_WW->Get("gpCos");
	gWpcos->SetTitle("WW p-cos correlation");
	gWpcos->Draw("CONT");
	//
	//HstF_HZ->Close();
	//HstF_ZZ->Close();
